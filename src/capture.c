...
void capture_loop(char *program_name, char *interface, char *file, char *filter,
  ...
  if (interface) {
    ...
    if (!handle) {
      ...
      /* Check if the error message corresponds to the PCAP_ERROR_NO_SUCH_DEVICE
       * message. */
      if (!strncmp(pcap_statustostr(PCAP_ERROR_NO_SUCH_DEVICE),
                   errbuf + sizeof(interface) - 2, 21))
        print_devices();
